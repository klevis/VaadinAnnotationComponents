 ...
 * class can be used as is or behind the scenes by using ViewBoundForm directly.
 * 
 */
public class ArmPreCreatedFieldsHelper<T> implements FormFieldFactory {

	private Object[] fieldSources;

	private HashMap<String, Field> hashMap = new HashMap<String, Field>();
	 ...
	 */
	public ArmPreCreatedFieldsHelper(Class<T> beanClass, Object... fieldSources) {
	...
	public void setNullReprent(Field field) {
		if (field instanceof AbstractTextField) {

			AbstractTextField abs = (AbstractTextField) field;
			abs.setNullRepresentation("");
	...
	public Field createField(Item item, Object propertyId, Component uiContext) {
		// pre-created-fields-pattern
		Field findField = findField(propertyId, fieldSources);

		if (findField != null) {
			getHashMap().put((String) propertyId, findField);

			setNullReprent(findField);
			BeanValidationValidator validator = BeanValidationValidator
					.addValidator(findField, propertyId, beanClass);
		}
		return findField;
	}
		...
		for (int i = 0; i < fields.length; i++) {
			...
			try {
				...
				if (reflectField.isAnnotationPresent(PropertyId.class)
						&& reflectField.getAnnotation(PropertyId.class).value()
	...
	public void setHashMap(HashMap<String, Field> hashMap) {
		this.hashMap = hashMap;
	}

	public HashMap<String, Field> getHashMap() {
		return hashMap;
	}
